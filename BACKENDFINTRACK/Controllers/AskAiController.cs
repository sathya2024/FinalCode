...
namespace PortfolioTrackerApi.Controllers
{
    ...
    public class AskAiController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromBody] AskAiRequest request)
        {
            ...
            // 1. Classify if it's a finance question
...
";
            ...
            {
                return Ok(new { answer = "Sorry, I can only answer questions related to the finance domain.", suggestions = new List<string>() });
            }

            // 2. Classify if it's a portfolio-specific question
...
";
            ...
            if (isPortfolioQuestion)
            {
                ...
                if (!request.UserId.HasValue)
                    return Ok(new { answer = "To answer questions about your portfolio, please provide your UserId.", suggestions = new List<string>() });
                ...
                if (!userInvestments.Any())
                    return Ok(new { answer = "Your portfolio is empty or unavailable.", suggestions = new List<string>() });
            ...
            // 4. Compose the AI prompt (always ask for suggestions)
            aiPrompt = $@"
User question: ""{request.Question}""
{(isPortfolioQuestion ? $"Portfolio:\n{sb}" : "")}
...
";
            ...
            var payload = new
            {
                ...
                messages = new[]
                {
                    new { role = "system", content = "You are a helpful financial assistant. After your answer, provide 2-3 follow-up questions the user might ask next. Format: Answer: <answer> Suggestions: 1. <q1> 2. <q2> 3. <q3>" },
            ...
            if (!string.IsNullOrWhiteSpace(answer))
            {
                ...
                {
                    if (line.Trim().StartsWith("Suggestions:", System.StringComparison.OrdinalIgnoreCase))
                    {
                        inSuggestions = true;
                        continue;
                    }
                    ...
                    else if (line.Trim().StartsWith("Answer:", System.StringComparison.OrdinalIgnoreCase))
                    {
                        answerBuilder.AppendLine(line.Substring("Answer:".Length).Trim());
                    }
                    else
                    {
                ...
                mainAnswer = answerBuilder.ToString().Trim();
            }

            return Ok(new { answer = mainAnswer, suggestions });
        }
    }
}
